...
export default function MatchResult({ group, expanded = false }: MatchResultProps) {
  const matchResult = calcMatchPlayResult(group.team1Holes, group.team2Holes);
  ...
  const formatLabel = group.format === "bestBall" ? "Best Ball" : "Alternate Shot";

  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      {/* Header */}
      <div className="bg-[#1a3c2b] text-white px-4 py-2 flex items-center justify-between">
        <span className="text-xs font-medium text-white/70">{formatLabel}</span>
